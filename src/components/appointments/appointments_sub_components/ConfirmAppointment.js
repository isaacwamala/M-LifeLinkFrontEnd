...
// Create the axios instance WITHOUT baking in the token at module-load time.
const apiClient = axios.create({
    baseURL: API_BASE_URL,
});

// Attach a fresh token + content-type on every single request.
// This fixes the "unauthenticated" bug: previously `token` was read once
// with `localStorage.getItem("access_token")` at import time and baked
// into axios.create()'s default headers. If this module was evaluated
// before the token existed in localStorage (or the token later changed),
// every request from this component kept using the stale/empty value
// forever (e.g. `Authorization: Bearer null`), even though other parts
// of the app that read the token inside the component body always
// picked it up fresh.
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function ConfirmAppointment({ appointment, onClose, onConfirmed }) {
    ...
    const handleConfirm = async () => {
        ...
        setError(null);
        try {
            const { data } = await apiClient.post("appointments/confirmAppointment", {
                id: appointment.id,
                status: "confirmed",
            });
                        ...
                        <>
                            ...
                            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-700">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">Confirm Appointment</h2>
                            ...
                            <div className="px-5 py-4 space-y-4">
                                ...
                                <div className="flex items-start gap-2.5 px-3.5 py-3 rounded-xl bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
                                    ...
                                    <p className="text-sm text-green-700 dark:text-green-300">
                                        You are about to confirm this appointment. The status will be updated to <span className="font-semibold">Confirmed</span>.
                                    </p>
                                </div>
