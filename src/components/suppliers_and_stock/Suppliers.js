...
function Suppliers() {
    ...
    const fetchSuppliers = async () => {
        try {
            ...
            if (response.data.success) {
                ...
                const formatted = response.data.suppliers.map(s => ({
                    id: s.id,
                    name: s.name,
                ...
                console.log("Suppliers loaded:", formatted);
    ...
    const handleAddAndEditSubmitSuppliers = async (e) => {
        ...
        if (!safeTrim(formData.contact_person)) {
            toast.error("Contact person is required.");
            return;
        }
        if (!safeTrim(formData.contact)) {
        ...
        try {
            ...
            } else if (modalMode === "edit" && currentSupplier) {
                await axios.post(
                    `${API_BASE_URL}config/updateSupplier`,
                    {
                            ...
                            {loading ? (
                                ...
                                currentSuppliers.map((supplier) => (
                                    ...
                                    >
                                        ...
                                        <p className="text-gray-500 dark:text-gray-300 text-sm">{supplier.number}</p>
                                        <p className="text-gray-700 dark:text-gray-200 mt-2">
                                            Contact: {supplier.contact_person}
                                        </p>
                                        <p className="text-gray-700 dark:text-gray-200 text-sm">Phone: {supplier.contact}</p>
                                        <p className="text-gray-700 dark:text-gray-200 text-sm">Email: {supplier.email}</p>
                                        <p className="text-gray-700 dark:text-gray-200 text-sm">Address: {supplier.location || "N/A"}</p>
                                        {/* <p className={`mt-3 font-medium ${supplier.balance < 0
                                            ? "text-red-500"
                                            : supplier.balance > 0
                                                ? "text-green-500"
                                                : "text-gray-500 dark:text-gray-300"
                                            }`}>
                                            Balance: UGX {supplier.balance.toLocaleString()}
                                        </p> */}
                            ...
                            <div className="relative p-4 w-full max-w-lg max-h-full">
                                <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700">
                                    ...
                                    <form onSubmit={handleAddAndEditSubmitSuppliers} className="p-4 space-y-4">
                                        {["name", "contact_person", "contact", "email", "location"].map(
                                            (field) => (
                                                <div key={field}>
